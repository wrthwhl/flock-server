...
export default {
    ...
    tripInfoChanged: {
      subscribe: withFilter(
        ...
        async (payload, { tripID }, { User, user: { email } }) => {
          ...
          const participants = updatedTrip.participants.map(participant =>
            String(participant)
          );
          return (
            participants.includes(user._id.toString()) &&
            (!tripID || String(tripID) === String(updatedTrip._id))
          );
        }
      )
    },
  ...
  Mutation: {
    createTrip: async (_, { trip }, { Trip, User, user }) => {
      const { newTrip, affectedUsers } = await createTrip(trip, user, {
        Trip,
        User
      });
    ...
    addParticipants: async (_, { tripID, participants }, { User, Trip }) => {
      let { updatedTrip, affectedUsers } = await addParticipants(
        tripID,
        participants,
        { User, Trip }
      );
    ...
    removeParticipants: async (_, { tripID, participants }, { Trip, User }) => {
      const { updatedTrip, affectedUsers } = await removeParticipants(
        tripID,
        participants,
        User,
        Trip
    ...
    addOrVoteForDestination: async (
      ...
      const updatedTrip = await addOrVoteForDestination(
        tripID,
        destinations,
        user,
        { Trip }
    ...
    addOrVoteForBudget: async (_, { tripID, budget }, { Trip, user }) => {
      const updatedTrip = await addOrVoteForBudget(tripID, budget, user, {
        Trip
      });
      pubsub.publish('TRIPINFO_CHANGED', { tripInfoChanged: updatedTrip });
      return updatedTrip;
    ...
    addOrVoteForTimeFrame: async (
      ...
      const updatedTrip = await addOrVoteForTimeFrame(
        tripID,
        timeFrames,
        user,
        { Trip }
    ...
    removeVoteForDestination: async (
      ...
      const updatedTrip = await removeVoteForDestination(
        tripID,
        suggestionID,
        user,
        Trip
    ...
    removeVoteForTimeFrame: async (
      ...
      const updatedTrip = await removeVoteForTimeFrame(
        tripID,
        suggestionID,
        user,
        Trip
    ...
    removeVoteForBudget: async (
      _,
      { tripID, suggestionID },
      { Trip, user }
    ) => {
      const updatedTrip = await removeVoteForBudget(
        tripID,
        suggestionID,
        user,
        Trip
    ...
    lockDestination: async (_, { tripID, suggestionID }, { Trip, user }) => {
      const updatedTrip = await lockDestination(
        tripID,
        suggestionID,
        user,
        Trip
    ...
    addGeneralMessage: async (_, { tripID, message }, { Trip, user }) => {
      const updatedTrip = await writeGeneralMessage(
        tripID,
        message,
        user,
        Trip
    ...
    addTimeFrameMessage: async (_, { tripID, message }, { Trip, user }) => {
      const updatedTrip = await writeTimeFrameMessage(
        tripID,
        message,
        user,
        Trip
    ...
    addDestinationMessage: async (_, { tripID, message }, { Trip, user }) => {
      const updatedTrip = await writeDestinationMessage(
        tripID,
        message,
        user,
        Trip
